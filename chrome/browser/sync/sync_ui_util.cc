...
base::string16 GetSyncedStateStatusLabel(
  ...
  if (!service || service->GetUserSettings()->IsFirstSetupComplete() ||
    ...
    if (service) {
      ...
      if (auth_error.state() != GoogleServiceAuthError::NONE) {
        if (status_label && link_label) {
          GetStatusForAuthError(profile, identity_manager, status_label,
                                link_label, action_type);
        }
        return SYNC_ERROR;
      }
      ...
      if (service->HasDisableReason(
        ...
        if (status_label) {
          status_label->assign(GetSyncedStateStatusLabel(
              service, identity_manager, style, sync_everything));
        }
        return PRE_SYNCED;
      }
    }
    ...
    if (status_label) {
      status_label->assign(GetSyncedStateStatusLabel(service, identity_manager,
                                                     style, sync_everything));
    }
    return SYNCED;
  } else {
